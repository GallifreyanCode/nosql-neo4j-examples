...
public final class FunctionalTestHelper {
	...
	public FunctionalTestHelper(NeoServerWithEmbeddedWebServer server) {
		if (server.getDatabase() == null) {
			throw new RuntimeException("Server must be started before using "
					+ getClass().getName());
		}
		this.server = server;
	}
	...
	public String indexNodeUri(String indexName, String key, Object value) {
		return indexNodeUri(indexName) + "/" + key + "/"
				+ value.toString().replace(" ", "%20");
	}

	public String indexRelationshipUri(String indexName, String key,
			Object value) {
		try {
			return indexRelationshipUri(indexName) + "/" + key + "/"
